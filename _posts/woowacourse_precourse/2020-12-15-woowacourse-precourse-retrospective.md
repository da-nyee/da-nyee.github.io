...
tags: [retrospective, woowacourse, precourse]
---
...
저는 11월 25일부터 3주 동안 `웹 백엔드` 분야로 참가하고 있고, 어느덧 마지막 미션을 완료했습니다.<br/>
총 3개의 미션을 수행하며 고민하고 생각했던 부분을 기록하기 위해 이번 포스팅을 작성합니다.<br/>

## <b>미션 1. 숫자 야구 게임</b>

첫번째 미션은 `숫자 야구 게임`이었습니다.<br/>
...
미션을 시작하기 앞서 `구현할 기능 목록`과 `확인할 프로그래밍 목록`을 작성했습니다.<br/>
...
1차 미션을 진행하며 가장 크게 신경썼던 부분은 `함수 분리`와 `상수 선언`입니다.<br/>

### <b>함수 분리</b>

먼저, 함수 분리는 프로그래밍 요구사항을 참고해서 진행했습니다.<br/>
요구사항에 `함수(또는) 메소드가 한 가지 일만 하도록 최대한 작게 만들어라.`라고 적혀 있어,<br/>
...
코드를 다시보니 `initflag()` 함수명을 축약해서 작성했는데.. 반성하게 됩니다.<br/>
...
`Type`이 가장 무난할 것 같아 `속해 있는 상수들을 일반화 할 수 있는 단어 + Type` 형태로 작성했습니다.<br/>

1차 미션의 기능 구현과 README 작성을 마치고,<br/>
프로그램을 전체적으로 테스트 한 후에 `Github PR`을 날려 미션을 완료했습니다! 😙

## <b>미션 2. 자동차 경주 게임</b>

두번째 미션은 `자동차 경주 게임`이었습니다.
...
2차 미션은 메일 내용을 바탕으로 `클래스 분리`, `네이밍`, `README 작성`에 집중했습니다.<br/>

### <b>클래스 분리</b>

먼저, 클래스 분리는 2차 미션 안내를 참고해서 진행했습니다.<br/>
미션 안내에 이번 미션의 목표가 적혀 있었는데, `함수 분리 + 클래스 분리`였습니다.<br/>
...
우승자를 알아내기 위해 `checkWinner()` 함수를 작성했고,<br/>
우승자가 존재하는지 알아내기 위해 `isNoWinner()`와 `isWinner()` 함수들을 작성했습니다.<br/>

우승자를 알아내기 위한 부분을 3차 미션까지 끝낸 시점에 다시 확인하니 리팩토링할 부분이 보입니다.<br/>
지금은 `checkWinner()` 내에서 if 문으로 자동차 위치와 최대 위치를 비교하는 형태로 구현되어 있습니다.<br/>
...
이처럼 `완성된 디렉토리 구조`에 전체 패키지와 파일들을 트리 형태로 나타냈습니다.<br/>

README에 직접 디렉토리 구조를 그려본 건 처음이었는데,<br/>
한눈에 알아보기 좋은 형태인 것 같아 앞으로 종종 활용해 볼 생각입니다. 😄<br/>

2차 미션도 기능 구현과 README 작성을 마치고,<br/>
프로그램 전체 테스트를 한번 더 진행한 후에 `Github PR`을 날려 미션을 완료했습니다! 😚<br/>

## <b>미션 3. 지하철 노선도</b>

세번째 미션은 `지하철 노선도`였습니다.
...
이번에는 `클래스 관계 맺기`, `StringBuilder() 활용`, `상속 활용`에 집중했습니다.

### <b>클래스 관계 맺기</b>

먼저, 클래스 관계 맺기는 3차 미션 안내를 참고해서 진행했습니다.<br/>
미션 안내에 이번 미션의 목표는 `함수 분리 + 클래스 분리 + 클래스 관계 맺기`라고 적혀 있었습니다.<br/>
따라서, 이를 바탕으로 `MVC(Model-View-Controller) 패턴`을 활용해서 코드를 작성하려고 노력했습니다.<br/>
...
전체 피드백 중에서<br/>
`객체의 상태를 보기 위한 로그 메시지 성격이 강하다면 toString()을 통해 구현한다`는 내용이 있었는데,<br/>
이 부분이 잘 이해되지 않아 구글링을 반복했습니다.<br/>

피드백에 첨부된 코드와 검색 결과를 바탕으로,<br/>
조회 기능을 구현할 때 `StringBuilder()`를 사용해야겠다는 결론을 내리게 되었습니다.<br/>
...
전체 지하철 노선의 상태를 확인하기 위해 `readTransitMap()` 함수를 선언했고,<br/>
객체를 출력 요구사항에 따라 출력하기 위해 StringBuilder()에 관련 값들을 append()해서 구현했습니다.<br/>

### <b>상속 활용</b>

마지막으로, 3차 미션에서 객체지향의 다형성을 살리고자 `상속`을 활용했습니다.<br/>
여러 클래스의 공통 함수는 `인터페이스`로 선언하고, 이를 구현 클래스에서 구현했습니다.<br/>
한 클래스에 중복되는 일부 함수는 `부모-자식` 관계로, 이를 자식 클래스에서 재정의했습니다.<br/>
...
`FeatureInterface()`에 추가, 삭제, 조회의 공통 로직이 추상 메소드로 선언되어 있습니다.<br/>
따라서, 이를 implements해서 현재 클래스에서 각 로직을 구현했습니다.<br/>
`SubwayService()`에는 기능 관리의 일부 공통 로직이 작성되어 있습니다.<br/>
...
프로그램 전체 테스트를 마지막으로 해 본 후에 `Github PR`을 날려 미션을 완료하겠습니다! 🤗<br/>
